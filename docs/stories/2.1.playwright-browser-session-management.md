...
| 2025-09-18 | 0.2 | Added browser session management infrastructure | James (Dev) |
...
- Implemented asynchronous browser session manager with concurrency controls and restart handling.
- Added stub fallback when Playwright is unavailable while tracking lifecycle metrics.
- Provided unit coverage for engine selection, concurrency limits, and metrics behaviour.

### File List

- src/legacy_web_mcp/browser/__init__.py
- src/legacy_web_mcp/browser/session.py
- tests/unit/browser/test_session.py
