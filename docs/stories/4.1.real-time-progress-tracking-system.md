...
## Tasks / Subtasks
- [x] Design `ProgressState` structure capturing per-page status and timestamps (AC: 1,7)
- [x] Instrument analysis workflow to push status updates and counts in real time (AC: 2)
- [x] Calculate rolling averages and ETA based on completed page durations (AC: 3)
- [x] Expose `analysis_progress` MCP resource streaming status updates (AC: 4)
- [x] Track failures and retry attempts with structured metadata for diagnostics (AC: 5)
- [x] Persist progress snapshots to disk for restart recovery (AC: 6)
- [x] Log progress state transitions with timestamps for performance auditing (AC: 7)
...
- Added `ProgressTracker` with per-page lifecycle tracking, ETA calculation, and JSON snapshot persistence under docs progress directory.
- Wired tracker into sequential workflow with checkpoint manager integration to stream updates and recover state.
- Registered `analysis_progress` MCP resource plus fallback server method to surface live status to IDE clients.
- Captured structured progress events for diagnostics and updated documentation metadata alongside snapshots.
...
- `tests/unit/progress/test_tracker.py`
