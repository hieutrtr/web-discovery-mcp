...
## Tasks / Subtasks
- [x] Create `interactive_analysis` flow orchestrating per-page prompts with user choices (AC: 1)
- [x] Display model cost estimates using data from Stories 3.1-3.2 before running Step 1 (AC: 2)
- [x] Render Step 1 summary for approval/edit/retry and persist user decisions (AC: 3)
- [x] Provide Step 2 validation checkpoint with accept/retry controls (AC: 4)
- [x] Allow users to adjust page queue ordering or skip/add URLs mid-session (AC: 5)
- [x] Surface runtime configuration tweaks (model, timeout) applied dynamically (AC: 6)
- [x] Integrate with checkpoint system to save state on every decision (AC: 7)
...
- Implemented `InteractiveAnalysisSession` coordinating user-driven decisions, queue management, and configuration updates.
- Added cost estimation through `LLMClient.estimate_cost` to surface per-step budget signals before execution.
- Captured approval/edit/retry loops for Step 1 and Step 2 with checkpoint writes and documentation refresh on each accepted page.

### File List
- `src/legacy_web_mcp/interaction/interactive.py`
- `src/legacy_web_mcp/interaction/__init__.py`
- `src/legacy_web_mcp/llm/client.py`
- `tests/unit/interaction/test_interactive.py`
