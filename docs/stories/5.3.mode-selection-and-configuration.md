...
## Tasks / Subtasks
- [x] Create `start_analysis` tool presenting interactive vs YOLO explanations (AC: 1,5)
- [x] Collect mode-specific configuration before delegating to respective workflows (AC: 2,6)
- [x] Load defaults from environment variables defined in Story 3.2/1.3 (AC: 3)
- [x] Implement runtime switch to transfer active queue from interactive to YOLO flow (AC: 4)
- [x] Persist mode choice and configuration for documentation/telemetry (AC: 5,6)
